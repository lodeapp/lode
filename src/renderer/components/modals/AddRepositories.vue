    ...
    <Modal :title="$root.project ? $string.set('Add repositories to :0', $root.project.name) : 'Add repositories'">
        <form class="add-repositories" @submit.prevent="handleSubmit">
            ...
            >
                <dd class="d-flex">
                    ...
                    >
                    <button class="btn btn-sm" type="button" @click="choose(index)">
                        Choose
                    </button>
                    <button class="remove-row tooltipped tooltipped-nw" type="button" aria-label="Clear row" @click="removeRow(index)">
                        <Icon symbol="x" />
                    </button>
                </dd>
                <dd v-if="slot.validator.hasErrors('path')" class="form-error">
                    {{ slot.validator.getErrors('path') }}
                </dd>
            </dl>
            <dl class="form-group">
            ...
            <dl class="form-group auto-scan">
                <label>
                    <input v-model="autoScan" type="checkbox" checked="checked">
        ...
        <template #footer>
            <div class="modal-footer tertiary separated">
                <button type="button" class="btn btn-sm" :disabled="loading" @click="cancel">
                    Cancel
                </button>
